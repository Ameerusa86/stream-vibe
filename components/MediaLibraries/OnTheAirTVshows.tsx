...
import { fetchOnTheAirTvShows } from "@/services/TMDBapi";
import { Movie, TvShow } from "@/Types/types";
import LibCard from "../Cards/LibCard";
import { LoadingComponent } from "../Loading";

const OnTheAirTVShows = () => {
  const [media, setMedia] = useState<TvShow[]>([]);
  ...
  const loadOnTheAirTVShows = async () => {
    setLoading(true);
    try {
      const mediaData = await fetchOnTheAirTvShows();
      setMedia(mediaData);
    } catch (error) {
      console.error(`Failed to load on the air tv shows`, error);
    } finally {
      setLoading(false);
    }
  };
